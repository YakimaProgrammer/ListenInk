...
const categoriesMapStateToProps = (state: RootState) => {
  ...
  const categories: ReshapedCategory[] =
...
function CategoriesComponent({
  ...
  status,
  reason,
}: CategoriesPropsFromRedux) {
  return (
    <Conditional status={status} reason={reason}>
      <List>
        {categories?.map((c) => (
          <ConnectedCategory key={c.id} c={c} />
        ))}
      </List>
    </Conditional>
  );
...
const categoryMapStateToProps = (
  state: RootState,
  ownProps: CategoryOwnProps
) => ({
  open: !!state.ui.openCategories[ownProps.c.id],
});
const categoryMapDispatchToProps = (
  dispatch: AppDispatch,
  ownProps: CategoryOwnProps
) => ({
  setOpen: (open: boolean) =>
    dispatch(setCategory({ open, id: ownProps.c.id })),
});
const categoryConnector = connect(
  categoryMapStateToProps,
  categoryMapDispatchToProps
);
...
function CategoryComponent({
  ...
  open,
  setOpen,
}: CategoryPropsFromRedux & CategoryOwnProps) {
  ...
  const documents = c.documents.map((d) => (
    <ListItemButton
      key={d.id}
      selected={d.id === docId}
      onClick={() => navigate(urlFor("docs", d.id))}
    >
  ...
  return (
    <>
      ...
      >
        <ListItemText style={{ color: c.color }}>{c.name}</ListItemText>
      </ListItemButton>
